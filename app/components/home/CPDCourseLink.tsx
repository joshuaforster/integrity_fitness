...
export default function CPDCourseLink({ course }: { course: CPDCourse }) {
  return (
    ...
    >
      <span className="text-xs font-medium text-white/80 group-hover:text-white pr-4 transition-colors duration-200">
        {course.title}
      </span>
      <span className="text-white/40 group-hover:text-[#CE1A19] group-hover:translate-x-1 transition-all duration-200 text-sm flex-shrink-0">
        →
      </span>
    </a>
  );
